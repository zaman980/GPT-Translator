...
import { Mic, ArrowRightLeft, Copy } from 'lucide-react'
...
export function TranslatorTool() {
  ...
  const startListening = () => {
    ...
    recognition.lang = fromLang === 'auto' ? 'en-US' : `${fromLang}-US`
  ...
  const handleTranslate = async () => {
    ...
    if (!apiKey || apiKey.startsWith('sk-')) {
      setOutputText('⚠️ Missing or invalid OpenAI API key. Add it in Vercel Environment Variables.')
      return
    }
    ...
    try {
      ...
      const result = data.choices[0]?.message?.content?.trim() || 'No translation received.'
      setOutputText(result)
    } catch (error: any) {
      console.error('Translation error:', error)
      setOutputText(`Translation failed: ${error.message || 'Check console/network.'}`)
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="py-16 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-6">
          {['Text Translate', 'Docx', 'XLSX', 'PDF', 'PPTX', 'Subtitle File', 'XML', 'JSON', 'YAML', 'CSV', 'Txt File', 'HTML'].map((item) => (
            <button
              key={item}
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {['Web-Page', 'Markdown', 'Email', 'Audio/Video', 'Handwritten Text'].map((item) => (
            <button
              key={item}
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-12">
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 mb-10">
            <select
              ...
              className="w-full md:w-auto px-8 py-4 border-2 border-gray-300 rounded-2xl text-lg font-medium focus:border-purple-500 outline-none"
            >
              {languages.map((lang) => (
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-10">
            ...
            <div className="relative">
              ...
              {outputText && !isLoading && (
                <button
                  onClick={copyText}
                  className="absolute top-4 right-4 z-20 flex items-center gap-2 px-4 py-2 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition font-medium"
                >
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <button
              ...
              className="px-20 py-5 bg-purple-700 text-white text-xl font-bold rounded-full hover:bg-purple-800 disabled:opacity-50 shadow-xl transition"
            >
              Translate
            </button>
          </div>
